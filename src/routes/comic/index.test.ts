...
describe('API test', () => {
	...
	it('should fetch the correct comic data for the given id', async () => {
		const realComic = {
			...
			title:
				'Know Your Vines\u003Cimg src="" onerror="alert(\'You\\\'ve been hacked! Should have used safe_title instead of title and textContent instead of innerHTML ¯\\\\_(ツ)_/¯\');this.parentNode.removeChild(this);"\u003E',
			day: '30'
		};
		const id = await fetchId('al.andukov@innopolis.university');
		const comic: ComicResponse = await fetch(
